 ...
 * Description: Calculates and displays dynamic package prices based on selected date and venue.
 * Version: 1.0.0
 * Author: Fabio
...
function dp_render_simple_log_page() {
    ...
    wp_nonce_field('dp_search_logs_action', 'dp_search_logs_nonce');
...
function dp_handle_csv_export() {
    if (
        ...
        $search = '';
if (
    isset($_GET['dp_search_logs_nonce']) &&
    wp_verify_nonce($_GET['dp_search_logs_nonce'], 'dp_search_logs_action')
) {
    $search = isset($_GET['s']) ? sanitize_text_field(wp_unslash($_GET['s'])) : '';
}
