...
// Handle form submit
if (isset($_POST['dynamic_pricing_package_nonce']) && wp_verify_nonce($_POST['dynamic_pricing_package_nonce'], 'save_dynamic_pricing_package')) {
    $id = isset($_POST['package_id']) ? intval($_POST['package_id']) : 0;
    $name = sanitize_text_field($_POST['name']);
    $base_price = floatval($_POST['base_price']);
    $midweek_discount_type = sanitize_text_field($_POST['midweek_discount_type']);
    $midweek_discount_value = floatval($_POST['midweek_discount_value']);
    $winter_discount_type = sanitize_text_field($_POST['winter_discount_type']);
    $winter_discount_value = floatval($_POST['winter_discount_value']);
    ...
    if ($id > 0) {
        ...
        $shortcode = '[dynamic_price id="' . $new_id . '"]';
...
// Handle delete
if (isset($_GET['delete_package']) && isset($_GET['_wpnonce']) && wp_verify_nonce($_GET['_wpnonce'], 'delete_dynamic_pricing_package')) {
            ...
            <?php if ($packages) : ?>
                <?php foreach ($packages as $package) : ?>
                    <tr>
                        ...
                        <td>
                            <a href="<?php echo admin_url('admin.php?page=dynamic-pricing-packages&edit_package=' . $package->id); ?>" class="button">Edit</a>
                            <a href="<?php echo wp_nonce_url(admin_url('admin.php?page=dynamic-pricing-packages&delete_package=' . $package->id), 'delete_dynamic_pricing_package'); ?>" class="button button-danger" onclick="return confirm('Are you sure you want to delete this package?');">Delete</a>
                        </td>
                    </tr>
                <?php endforeach; ?>
            <?php else : ?>
